  ...
  <div ref="googleMap" :class="_classes" :style="style">
    <div v-if="ready">
      ...
      <google-map-marker
        ...
        :marker="marker"
        :key="marker.id"
        :callbacks="{ click: () => click(marker) }"
      />
      <google-map-info-window
        v-if="infoWindowContent.position.lat"
        :info-window="infoWindowContent"
      />
    </div>
    <div v-else>
...
import GoogleMapMarker from "./Shapes/GoogleMapMarker.vue";
import GoogleMapPolygon from "./Shapes/GoogleMapPolygon.vue";
import GoogleMapPolyline from "./Shapes/GoogleMapPolyline.vue";
import GoogleMapRectangle from "./Shapes/GoogleMapRectangle.vue";
import GoogleMapInfoWindow from "./Shapes/GoogleMapInfoWindow.vue";
...
export default class GoogleMap extends Vue {
  ...
  @Prop() polylines?: { id: string; paths: any };
  ...
  onMarkersChanged() {
    if (this.markers) {
      const marker = this.markers.find(m => m.id === this.infoWindowContent.id)
      if (marker && !marker.visible) {
        this.infoWindowContent.position = {}
      }
    }
  }

  public get style() {
    const height = this.aspectRatio
      ? `calc(100vw / ${this.aspectRatio})`
      : "100%";
  ...
  protected _mapConfig() {
    return {
      ...
      ...(this.zoomControl !== 0
        ? { scaleControl: false, zoomeControl: false }
        : {}),
  ...
  async mounted() {
    this._api = await loadNow(
      "places",
      "AIzaSyDRdC5ayB2snUW37qQP_6gHGVuxwbvlOIo"
    );
    await this.draw();
