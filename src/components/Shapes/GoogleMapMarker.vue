...
export default class GoogleMapMarker extends GoogleMapExtension {
  ...
  @Watch('marker') onMarkerChange() {
    if (this._marker) {
      this._marker.setOptions(this.marker as IMarkerOptions)
    }
  }
  ...
  async mounted() {
    ...
    if (!this.marker) {
      console.warn(
        `A GoogleMapMarker component was instantiated without any marker config!`,
        { context: this }
      );
    }
    ...
    if (marker) {
      ...
      markerEvents.forEach(evt => {
        if (this.callbacks && this.callbacks[evt]) {
          this.listeners.click = (this._marker as Marker).addListener(
            evt,
            this.callbacks[evt]
          )
        }
  ...
  beforeDestroy() {
    if (this._marker) {
      // remove from map
      // TODO: do we need to remove event listemer here? Does google api do that automatically?
